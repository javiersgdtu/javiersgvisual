...
/*function colorCharactersScale() {
    d3.json("data/data_out/characters.json").then(function(data) {

        var palette = data.map(function(d) {
            return d.Color
        })
        ...
        colorCharacter = d3.scaleOrdinal(palette).domain(characters);
...
//console.log(colorCharactersScale());
//var characters = [...new Set(data.map(d => d.Character))] //.sort()

var palette = Characters.map(function(d) {
    return d.Color
});
var characters = Characters.map(function(d) {
    return d.Character
});
var colorCharacter = d3.scaleOrdinal(palette).domain(characters);
console.log(colorCharacter("Ki Woo"))

//console.log(characters)
var characters_leg = Characters.filter(function(d) { return (d.Character != ("First chaufer") & d.Character != ("Party attendants")) });

console.log(characters_leg)
