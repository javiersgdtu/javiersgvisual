    ...
    d3.json("data/data_out/scenes.json").then(function(nodes) {
        ...
        nodes = nodes.map(function(d, i) {
            return {
                ...
                formattime: sectomin(d.difference),
                action: d.Movement
                    //category: i % 3
            }
        });
        ...
        var rectwidth = ((widthLong / 2) / xScenes.length) / 2;
        ...
        var legend_rect = legend.selectAll('rect')
            ...
            .on("click", function(event, d) {
                ////console.log(node)
                var i = legend_rect.nodes().indexOf(this) //new way of getting the index in d3 v6
        ...
        function restart() {
            ...
            node = node.enter().append("circle")
                ...
                .on("mouseover", function(event, d) {
                    ...
                    var pointxy = d3.pointer(event) //if i dont store it in a variable, then it doesn't work below. it sends it to the bottom of it.
                    tooltip
                        .html("<p style='text-align:center;margin:2px'><b>Character:</b> " + d.character + "</p><b>Location:</b> " + d.location + ". </br><b>Duration in Location</b>: " + d.formattime + "</br><b>Action:</b> " + d.action)
                        .transition() //If I dont include the transition there is no legend change from one circle to another after mouseover 
                        .duration(200)
                        .style("visibility", "visible")
                        .style("opacity", .9)
                        .style("left", pointxy[0] + "px") //event.pageX
                        .style("top", pointxy[1] + "px") //event.pageY
        ...
        /*   function handlemouseOver(i) {
               ...
               tooltip.transition()
                   ...
                   .style("visibility", "visible")
                   .style("opacity", .9)
                   .style("left", (d3.event.pageX + 10) + "px") //d3.event.pageY
                   .style("top", (d3.event.pageY - 30) + "px") //d3.event.pageY
                   .text(textmouseover[i]);
           }*/
    });
}